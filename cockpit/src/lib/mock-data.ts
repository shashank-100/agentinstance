...
const stripeDiff: FileDiff[] = [
  {
    ...
    lines: [
      ...
      { kind: "ctx", old: 18, new: 18, text: "  const event = verifySignature(rawBody, signature);" },
      ...
      { kind: "add", new: 24, text: "      const tier = resolveTier(sub.items.data[0]?.price.id);" },
      ...
      { kind: "add", new: 32, text: "      await setPlan(event.data.object.customer as string, 'free');" },
  ...
  },
  {
    ...
    lines: [
      ...
      { kind: "add", new: 2, text: "  it.each(['starter', 'pro', 'scale'])('maps %s price to tier', async (tier) => {" },
      { kind: "add", new: 3, text: "    await handle(fixture('sub.updated', { price: `price_${tier}_monthly` }));" },
...
const cacheDiff: FileDiff[] = [
  {
    ...
    lines: [
      ...
      { kind: "add", new: 41, text: "  const [existing] = await client.multi().get(key).ttl(key).exec();" },
      { kind: "add", new: 42, text: "  if (existing) return decode(existing);" },
      { kind: "add", new: 43, text: "  const fresh = await singleflight(key, loader);" },
      { kind: "ctx", old: 44, new: 44, text: "  await client.set(key, JSON.stringify(fresh), 'EX', ttl);" },
